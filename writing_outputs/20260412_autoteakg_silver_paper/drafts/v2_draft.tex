...
\subsection{Tea bioactivity reviews and evidence synthesis}
...
\subsection{Processing-aware tea chemistry and microbiome mechanisms}
...
\subsection{Tea databases, natural-product resources, and knowledge graphs}
...
\centering
\small
\caption{Reproducible construction stages for AutoTeaKG-Silver v1 and KG v3. Counts refer to the current manuscript data release.}
\label{tab:pipeline_stages}
\begin{tabular}{p{0.23\linewidth}p{0.27\linewidth}p{0.25\linewidth}p{0.15\linewidth}}
\toprule
Stage & Main script or artifact & Primary output & Count or result \\
\midrule
PubMed retrieval archive & search archive & query logs and batch summaries & 5 query blocks \\
Auto-only ideation and profiling & auto-only ideation script & auto-only data profile & 596 GLM5 records \\
Silver KG construction & silver-build script & base silver KG directory & 635 records \\
Abstract-level context extraction & targeted context extractor & abstract-patch directory & 365 patches \\
Vocabulary normalization & vocabulary normalizer & normalized context directory & 0 unmapped labels \\
PMC methods retrieval & PMC methods retriever & methods-section cache & 151 records with methods text \\
Methods-level extraction & methods batch runner & methods-patch directory & 151 patches \\
Final KG v3 figures & query-table and figure script & query tables and figures & 1,989 nodes; 8,195 edges \\
\bottomrule
\end{tabular}
\end{table}
...
The current manuscript release is organized so that each result can be traced to a script and output directory (Table~\ref{tab:pipeline_stages}). The final graph used for all Results figures is stored at \path{reports/methods_processing_vocab_normalized}. Its record table is \path{autoteakg_silver_records.csv}; graph files are \path{kg_v3/nodes.csv} and \path{kg_v3/edges.csv}. Figure-ready query tables are stored in \path{reports/kg_v3_query_tables}. Publication figures are regenerated by \path{scripts/generate_kg_v3_query_tables_and_figures.py} and exported as PDF, PNG, and SVG under \path{figures/kg_v3}. The paper draft project copies the figure PDFs and query tables into \path{writing_outputs/20260412_autoteakg_silver_paper}.
